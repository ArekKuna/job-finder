...
export const MyProfile = () => {
  ...
  const { data } = useCustomQuery<GetMeResponseDto>(GET_ME_URL, true, ['getMe']);
  ...
  return (
    <div className="pt-4">
      <div className="flex flex-col items-center gap-2">
        {avatarUrl ? (
          <img className="h-48 w-48 rounded-full" src={avatarUrl} alt="user avatar" />
        ) : (
          <ProfileIcon />
        )}

        <button className="text-jf-purple-400" onClick={() => fileInputRef.current?.click()}>
